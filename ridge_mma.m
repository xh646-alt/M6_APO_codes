function [w] = ridge_mma(X_train, y_train, lambda)

% Function to perform Ridge regression using the Moving Asymptotes (MMA) optimization method
%
% Inputs:
...
%   lambda  - regularization parameter that controls the weight shrinkage
%
% Outputs:
...
objective_summarize=zeros(maxIter,3);
...
while Iter<maxIter
    ...
    ObjFun=ObjFac'*(residual.^2)/NumT_temp+0.5*lambda*(ConsFac'*(w.^2));

    ObjFun_diff=(-2/NumT_temp*residual'*X_train)'+lambda*ConsFac.*w;
