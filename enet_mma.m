function [w]=enet_mma(X_train,y_train,lambda)

% Function to perform Elastic Net regression using the Moving Asymptotes (MMA) optimization method
%
% Inputs:
...
while Iter<maxIter
    ...
    ObjFun = ObjFac'*(residual.^2)/NumT_temp+lambda*(1-rho)*(ConsFac'*abs(w))+0.5*lambda*rho*(ConsFac'*(w.^2));

    ObjFun_diff=(-2/NumT_temp*residual'*X_train)'+lambda*(1-rho)*ConsFac.*sign(w)+lambda*rho*(ConsFac.*w);
